    ...
    with open(filepath, 'r') as f:
        for line in f:
            ...
            # Detect start of force block
                ...
                parts = line.split()
                # Stop when reaching summary lines
                if not parts[1].isdigit():
                    break

                fx, fy, fz = [float(x) * Hartree / Bohr for x in parts[2:5]]
                forces.append([fx, fy, fz])

    if not scf_converged:
        print(f"Skipping {filepath} — SCF did not converge.")
        return None

    if energy is None or not forces:
        print(f"Skipping {filepath} — energy or forces not found.")
        return None
...
def collect_cp2k_results(run_prefix='run', structure_file='structure.xyz', farming_prefix='FARMING_OUT_', output='cp2k_farmed_dataset.xyz'):
    ...
    for run_dir in sorted(os.listdir()):
        ...
        try:
            ...
            print(f"Failed to read {structure_path}: {e}")
            continue
