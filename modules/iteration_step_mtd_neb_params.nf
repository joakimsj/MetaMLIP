...
include { runMACE_MTD; runMACE_NEB; extractReact; collectNEBs; calcREF; updateDataset; reTrainMACE } from './processes_params.nf'
...
workflow ITERATION_STEP {
    ...
    main:
        ...
        mtd_out = runMACE_MTD(
            ...
            file(params.input.initial_structure),
            dataset_ch,
            models_ch,
            label_ch
        )
        ...
        extract_out.reaction_windows.view { "DEBUG: NEB INPUT WINDOWS => $it" }
        
        neb_inputs = extract_out.reaction_windows.flatten()

        neb_inputs.view { "DEBUG: NEB INPUTS => $it" }
        ...
        neb_out.neb_uncertain_frames.collect()
            .view { "DEBUG: ALL NEB FRAMES => $it" }
        ...
        calcREF_out = calcREF(
            ...
            file(params.cp2k.method_template),
        ...
        seed_ch = Channel.of( 1..1000000 )
            .randomSample( 3 )
            .view()
        foundation_model = file(params.input.foundation_model)
